...
public class RunIterator implements Iterator {
  ...
  private RunListNode iterObject;
  ...
  public int[] next() {
    ...
    int[] arr = new int[4];
    int length, red, green, blue;
    // Replace the following line with your solution.
    if(hasNext()==false){
      throw new NoSuchElementException();
    }
    else{
      length = this.iterObject.getLength();
      red = this.iterObject.getRed();
      green = this.iterObject.getGreen();
      blue = this.iterObject.getBlue();
      arr = new int[] {length, red, green, blue};
